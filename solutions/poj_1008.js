...
function howManyPassed(currentMonthName, currentDate, year) {
    return Haab.indexOf(currentMonthName) * 20 + currentDate + year * 365;
}
...
function showTzolkinDate(num) {
    var d = num % 13 + 1;
    var c = Tzolkin[num % 20];
    var y = ~~(num / 260);
    console.log(d + ' ' + c + ' ' + y);
}
...
function handle(str) {
    var a = str.split(' ');
    var currentDate = a[0].replace('.', '') * 1;
    var currentMonthName = a[1];
    var year = a[2] * 1;
    var num = howManyPassed(currentMonthName, currentDate, year);
    showTzolkinDate(num);
}
